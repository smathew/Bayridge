class LoanappsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @loanapp.save
        flash[:notice] = 'Thank You. Your Loan application has been submitted. <br/ >The details submitted are as below:'
		
		# Tell the Emailer to send an Email after save
		Emailer.deliver_loanapp_email(@loanapp)

        format.html { redirect_to(@loanapp) }
        format.xml  { render :xml => @loanapp, :status => :created, :location => @memberapp }
      else
  ...
  def update
    ...
    respond_to do |format|
      if @loanapp.update_attributes(params[:loanapp])
        flash[:notice] = 'Loan Application was successfully updated.'
        format.html { redirect_to(@loanapp) }
        format.xml  { head :ok }
      else
